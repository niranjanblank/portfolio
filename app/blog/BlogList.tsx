...
export async function BlogList({page, limit}: {page:number, limit: number})
 {
    ...
    return (
        ...
        {(page*limit)<blogData.total && (
            <Link href={`/blog?page=${page+1}&limit=${limit}`} className="flex items-center hover:bg-gray-200 dark:bg-zinc-800 
            hover:dark:bg-zinc-700
            dark:text-white p-2 px-4 rounded-md" >
            Next <MdNavigateNext className="text-2xl"/>
        </Link>
        )}
        {page!==1 && (
        <Link href={`/blog?page=${page>1?page-1:page}&limit=${limit}`} className="flex items-center justify-center hover:bg-gray-200 
